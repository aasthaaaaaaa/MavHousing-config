...
async function bootstrap() {
  const config = new DocumentBuilder()
    .setTitle('AuthServer')
    .setDescription('API Playground for authentication services')
    .addBearerAuth()
    .build();
  ...
  console.log(
    `Server started at ${process.env.HOST ?? '127.0.0.1'}:${process.env.port ?? 3004}/auth`,
  );
  console.log(
    `Swagger started at ${process.env.HOST ?? '127.0.0.1'}:${process.env.port ?? 3004}/api`,
  );
}
bootstrap();
