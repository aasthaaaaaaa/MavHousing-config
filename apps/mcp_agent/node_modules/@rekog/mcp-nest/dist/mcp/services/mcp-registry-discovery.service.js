...
var McpRegistryDiscoveryService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.McpRegistryDiscoveryService = void 0;
...
let McpRegistryDiscoveryService = McpRegistryDiscoveryService_1 = class McpRegistryDiscoveryService {
    constructor(discovery, metadataScanner, modulesContainer, options) {
        this.discovery = discovery;
        this.metadataScanner = metadataScanner;
        this.modulesContainer = modulesContainer;
        this.options = options;
        this.discoveredCapabilitiesByMcpModuleId = new Map();
        this.logger = (0, mcp_logger_factory_1.createMcpLogger)(McpRegistryDiscoveryService_1.name, this.options);
    }
    ...
    addDiscoveryTool(mcpModuleId, methodRef, token, methodName) {
        ...
        const guards = Reflect.getMetadata(decorators_1.MCP_GUARDS_METADATA_KEY, methodRef);
    ...
    getTools(mcpModuleId) {
        return (this.discoveredCapabilitiesByMcpModuleId
            .get(mcpModuleId)
    ...
    registerDynamicCapability(mcpModuleId, capability) {
        if (!this.discoveredCapabilitiesByMcpModuleId.has(mcpModuleId)) {
            this.discoveredCapabilitiesByMcpModuleId.set(mcpModuleId, []);
        }
        const capabilities = this.discoveredCapabilitiesByMcpModuleId.get(mcpModuleId);
        const existingIndex = capabilities.findIndex((c) => c.type === capability.type && c.methodName === capability.methodName);
        if (existingIndex !== -1) {
            this.logger.warn(`Dynamic ${capability.type} '${capability.methodName}' already registered in module: ${mcpModuleId}. Overwriting.`);
            capabilities[existingIndex] = capability;
        }
        else {
            this.logger.debug(`Dynamic ${capability.type} registered: ${capability.metadata['name']} in module: ${mcpModuleId}`);
            capabilities.push(capability);
        }
    }
    removeDynamicCapability(mcpModuleId, type, identifier) {
        const capabilities = this.discoveredCapabilitiesByMcpModuleId.get(mcpModuleId);
        if (!capabilities) {
            return;
        }
        this.logger.debug(`Dynamic ${type} removed: ${identifier} in module: ${mcpModuleId}`);
        this.discoveredCapabilitiesByMcpModuleId.set(mcpModuleId, capabilities.filter((c) => !(c.type === type && c.methodName === identifier)));
    }
};
exports.McpRegistryDiscoveryService = McpRegistryDiscoveryService;
exports.McpRegistryDiscoveryService = McpRegistryDiscoveryService = McpRegistryDiscoveryService_1 = __decorate([
...
//# sourceMappingURL=mcp-registry-discovery.service.js.map