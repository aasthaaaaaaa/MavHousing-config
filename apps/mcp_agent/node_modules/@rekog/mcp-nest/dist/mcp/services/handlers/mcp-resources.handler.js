...
let McpResourcesHandler = McpResourcesHandler_1 = class McpResourcesHandler extends mcp_handler_base_1.McpHandlerBase {
    constructor(moduleRef, registry, reflector, mcpModuleId, options) {
        super(moduleRef, registry, reflector, McpResourcesHandler_1.name, options);
    ...
    registerHandlers(mcpServer, httpRequest) {
        ...
        mcpServer.server.setRequestHandler(types_js_1.ReadResourceRequestSchema, async (request) => {
            ...
            try {
                ...
                return this.handleError(error, (resourceInfo?.resource ?? resourceTemplateInfo?.resourceTemplate), httpRequest);
            }
        });
    }
    async handleRequest(httpRequest, providerClass, uri, context, requestParams, methodName) {
        if (providerClass === mcp_dynamic_registry_service_1.DYNAMIC_RESOURCE_HANDLER_TOKEN) {
            const handler = mcp_dynamic_registry_service_1.McpRegistryService.getResourceHandlerByModuleId(this.mcpModuleId, methodName);
            if (!handler) {
                throw new types_js_1.McpError(types_js_1.ErrorCode.MethodNotFound, `Handler not found for dynamic resource: ${uri}`);
            }
